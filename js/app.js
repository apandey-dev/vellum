        ...
        try {
            ...
            saveTimeout = setTimeout(async () => {
                ...
                if (meta && meta.notesIndex[note.id]) {
                    const path = meta.notesIndex[note.id].path;
                    queueSync({ type: 'UPDATE', payload: { path, content } });
                }
            }, 1000);
...
async function createNote(title, folderId = null) {
    ...
    const path = `n/${cleanFolder.charAt(0)}/${cleanFolder.charAt(1) || '0'}/${id}.md`;
    ...
    queueSync({ type: 'UPDATE', payload: { path, content: '' } });
    queueSync({ type: 'INDEX', payload: { path: 'metadata/index.json', content: JSON.stringify(meta, null, 2) } });
...
async function deleteNote(noteId) {
    ...
    if (path) queueSync({ type: 'DELETE', payload: { path, content: null } });
...
async function deleteFolder(folderId) {
    ...
    if (meta) {
        ...
        notesToDelete.forEach(nid => {
            ...
            if (p) queueSync({ type: 'DELETE', payload: { path: p, content: null } });
...
document.addEventListener('sync-completed', async () => {
    console.log("Sync dispatch received, refreshing UI...");
    const oldActiveNote = activeNoteId;
    await fetchInitialData();
    if (notes.find(n => n.id === oldActiveNote)) {
        activeNoteId = oldActiveNote;
        loadActiveNote();
    }
});
...
function getNotesInFolder(folderId) {
    ...
    async function updateShareUI(noteId, container) {
        ...
        if (isPublic) {
            ...
            linkInput.value = note.public_id || '';
        } else {
        ...
        modalManager.openModal('share-note', {
            ...
            onMount: (container) => {
                ...
                shareToggle.onclick = async () => {
                    ...
                    if (newPublicStatus && (!note.public_id || !note.public_id.includes('gist.github.com'))) {
                        // Make POST to Gist
                        shareToggle.style.opacity = '0.5';
                        try {
                            const payload = {
                                description: `Shared from Vellum: ${note.title}`,
                                public: true,
                                files: {
                                    [note.title + '.md']: { content: note.content || 'Empty Note' }
                                }
                            };
                            ...
                            note.public_id = response.html_url;

                            // Update the shared_notes.json file
                            const sharedContent = await GitHubAPI.getFileContent('shared/shared_notes.json');
                            const sharedArr = sharedContent ? JSON.parse(sharedContent) : [];
                            sharedArr.push({ id: note.id, url: note.public_id });

                            queueSync({ type: 'SHARED', payload: { path: 'shared/shared_notes.json', content: JSON.stringify(sharedArr, null, 2) } });
                        ...
                        shareToggle.style.opacity = '1';
                    }

                    const meta = await DBStore.getMeta('index');
                    if (meta && meta.notesIndex[note.id]) {
                        meta.notesIndex[note.id].is_public = note.is_public;
                        meta.notesIndex[note.id].public_id = note.public_id;
                        await DBStore.setMeta('index', meta);
                        queueSync({ type: 'INDEX', payload: { path: 'metadata/index.json', content: JSON.stringify(meta, null, 2) } });
                    }
